...
def save_attachment(message_id: str, name: str, size: int | None, data_url: str | None):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO attachments (message_id, name, size, data_url) VALUES (?, ?, ?, ?)",
            (message_id, name, size, data_url)
        )
        conn.commit()
    finally:
        conn.close()
...
def get_session_messages(session_id: str):
    ...
    try:
        ...
        for msg in messages:
            cursor.execute("SELECT name, size, data_url FROM attachments WHERE message_id = ?", (msg["id"],))
